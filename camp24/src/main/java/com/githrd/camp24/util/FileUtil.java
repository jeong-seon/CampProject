...
//		System.out.println("####### FileUtil path : " + path);
	...
	public void setList() {
		...
		// 전송된 파일데이터의 키값만 뽑아본다.
		Enumeration en = multi.getFileNames();	// 전송된 파일들의 키값만 추출
		while(en.hasMoreElements()) {
			String key = (String) en.nextElement();
//			System.out.println("######### key : " + key);
			String oriname = multi.getOriginalFileName(key); // 업로드한 원래이름
...
//		System.out.println("###### bPath1 : " + bPath);
		bPath = bPath.substring(0, bPath.indexOf("/source")) + 
							"/source/camp24/src/main/webapp/resources/upload";
//		System.out.println("###### bPath2 : " + bPath);
	...
	public static String rename(String path, String oldName) {
		...
		while(file.exists()) {
			// 이 경우는 이미 newName 으로 저장된 파일이 존재하는 경우이므로
			// 파일의 이름을 다시 만들어야 한다.
			count++;
