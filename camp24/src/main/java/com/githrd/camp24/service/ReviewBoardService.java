...
public class ReviewBoardService {
	...
	public FileVO uploadProc(MultipartFile file/* , String dir */) {
		/*
		 * 이 함수가 파일을 업로드 하기 위해서는 컨트롤러에서 업로드할 파일의 정보를 받아와야 한다.
		 * 
...
//		fVO.setDir("/www" + dir);
	...
	public void editBoard(BoardVO bVO) {
		if (bVO.getRtitle() != null || bVO.getRbody() != null) {
			rDao.editBoard(bVO);
		}
		// 파일 정보 테이블에 파일정보들을 입력하고(반복작업 필요)
		if (bVO.getFile() != null) {
			ArrayList<FileVO> list = uploadProc(bVO.getFile());
			// rno 를 꺼내서 FileVO 들에 채워주고
			for (FileVO f : list) {
				f.setRno(bVO.getRno());
			}
